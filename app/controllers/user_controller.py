...
class Users(Controller):
    ...
    async def register_view(self, request: Request) -> Response:
        ...
        """
        return self.view(
            model={"title": "Inscription", "error": None, "form_data": {}}, request=request
        )
    ...
    async def login_view(self, request: Request):
        ...
        return self.view(
            "login_view",
            model={"title": "Connexion", "error": None, "identifier": ""},
            request=request,
        )
    ...
    async def reset_password_view(self, token: str, request: Request) -> Response:
        ...
        if not is_valid:
            if message == "expired" and user:
                return self.view(
                    "reset_password_expired", model={"email": user.email}, request=request
                )
